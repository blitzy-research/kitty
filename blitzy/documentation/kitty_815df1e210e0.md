...
**Redaction & reproducibility policy.** Runtime identifiers (PIDs, TIDs, the fd number, `/dev/pts/N`, the X window id) are **process-specific** to this session and are reported exactly as observed. Two classes of text are redacted to avoid leaking environment metadata, always with an explicit label that **preserves the observed byte length** so the reported `read()` counts remain verifiable: (1) the container **hostname** (shown as `[hostname redacted]`), and (2) the disposable-build **cwd path** that the shell places in its OSC-2 title (shown as `<cwd:NN B redacted>`). Separately, and purely to reduce noise, a few **non-material surrounding fields are shown abbreviated with `...`**: the timestamps in `ls -l` listings (and, for the launcher only, its size); the Kubernetes **pod-slice path prefix** that precedes the container id in the `cgroup` line (shown as `/../`); and the verbose **Go package-compilation** stretch of the build log. **No value the document actually reports is ever abbreviated** — the `fast_data_types.so` size `1253792`, the master-fd symlink target `/dev/pts/ptmx`, the container id, and every returned `read()` byte count are shown in full. `strace` trace blocks are quoted verbatim, the build tool's own `Linking ...` progress lines are quoted as emitted, and the only `"..."` inside a quoted strace payload is strace's own `-s` truncation marker (disclosed in §3); where the prose walk-throughs re-quote a read in abbreviated form (`…`), the complete line appears in the adjacent raw-trace block.
