...
Before creating the GLFW window, kitty issues a small number of `glfwWindowHint()` calls to specify what kind of OpenGL context it needs. The actual hints set in `create_os_window()` on Linux are:
...
Note that kitty does **not** explicitly set `GLFW_OPENGL_PROFILE` here — the core-profile behaviour of the resulting context is inherited from `GLFW_OPENGL_FORWARD_COMPAT=true` combined with a requested context version ≥ 3.2. (The symbol `GLFW_OPENGL_PROFILE` does appear elsewhere in `kitty/glfw.c` at line 2492 as `ADDC(GLFW_OPENGL_PROFILE)`, but that is a Python-binding constant registration, not a window-hint call.) No depth or stencil buffers are requested (kitty is a 2D UI and does not need them). On X11 the sRGB-capable framebuffer hint is set so that the `GL_FRAMEBUFFER_SRGB` pathway works correctly; however, on Wayland the sRGB hint is **explicitly disabled** because of known NVIDIA/Mesa bugs that produce incorrect colors when the hint is set — this conditional behavior lives in `create_os_window()` next to the other hints.
...
- **Discovery — `kitty/fontconfig.c`.** On Linux, the FontConfig library is not a hard link-time dependency of the `fast_data_types` extension; instead, it is loaded at runtime via `dlopen()` in `load_fontconfig_lib()` (line ~79), which tries a list of candidate library names in order — `"libfontconfig.so"` first, then `"libfontconfig.so.1"` as a fallback (the latter covers installs that lack the unversioned `.so` symlink) — each opened with `RTLD_LAZY`. The pointers to `fc_list`, `fc_match`, and `fc_match_postscript_name` are resolved lazily and exposed via a small Python-accessible helper surface. Pattern-to-dict conversion is used to reduce an opaque `FcPattern*` to a Python-accessible dictionary of attributes (`family`, `style`, `slant`, `weight`, `file`, `index`, etc.), which the Python side of the font stack (in `kitty/fonts/render.py`) consumes.

- **Rasterization — `kitty/freetype.c`.** The `Face` Python type wraps a FreeType `FT_Face`. `cell_metrics()` at lines ~387–405 is the function that computes every per-face pixel-accurate metric kitty needs: `cell_width`, `cell_height`, `baseline`, `underline_position`, `underline_thickness`, and strikethrough offsets. `calc_cell_width()` (lines ~374–384) iterates ASCII 32–127 loading each glyph and returns the **maximum** `horiAdvance / 64` observed — using `MAX` across the printable ASCII range guards against monospace fonts that cheat on a handful of glyphs. `calc_cell_height()` (lines ~141–154) computes `ascender − descender + line_gap` in font units, converts to pixels, and grows the height if the underscore glyph's bounding box extends below the computed baseline (so underscores never get clipped). Variable-font axes are supported through the same face wrapper.
...
15. **Font Group Init** — `load_fonts_data()` → `kitty/fonts.c:font_group_for()` (line ~204) → `initialize_font_group()` (line ~1495) → `calc_cell_metrics()` (line ~373)
...
- **DPI feeds `load_fonts_data(font_size, xdpi, ydpi)`** in `kitty/fonts.c`. `font_group_for()` (line ~204) looks up a `FontGroup` by `(font_sz_in_pts, dpi_x, dpi_y)`; on cache miss, `initialize_font_group()` (line ~1495) creates one. Internally, `calc_cell_metrics()` (line ~373) invokes `kitty/freetype.c:cell_metrics()` (lines ~387–405) which uses `calc_cell_width()` (lines ~374–384) and `calc_cell_height()` (lines ~141–154) to produce pixel-accurate cell dimensions from the font's FT_Face metrics.
...
The full expression (a) clamps the subtraction `self->ascender - self->underline_position` at zero via `MAX(0, …)` to prevent negative font-unit values, and (b) converts the clamped font-unit result to pixels via the helper `font_units_to_pixels_y()` before the outer `MIN(cell_height - 1, …)` applies the final upper bound.
...
- `kitty/debug_config.py` — Diagnostic reporting. Key functions: `compositor_name()` (lines ~206–230), `debug_config()`, `opengl_version_string()`, `current_fonts()`, `wayland_compositor_data()`.
...
- `kitty/state.h` — Process and per-window state structs (401 lines). Defines `OSWindow` (lines 216–256), `GlobalState` (immediately following the `OSWindow` struct), and the function prototypes that cross between `kitty/state.c`, `kitty/glfw.c`, and the Python binding layer.
...
- `kitty/shaders.py` — GLSL source loading, preprocessor handling, and per-variant compilation (204 lines). Implements `MultiReplacer` for `{GLSL_VERSION}` token substitution, `load_shader_programs()` (all four cell variants and three graphics variants), `load_bgimage_program()`, and `load_tint_program()`.
- `kitty/borders.py` — Border rendering glue. Defines `load_borders_program()` (line 63), which is **not** in `kitty/shaders.py` as one might expect; `kitty/main.py:load_all_shaders()` imports both `kitty/shaders.py:load_shader_programs()` and `kitty/borders.py:load_borders_program()` and co-invokes them during Step 19 of the initialization chain.

### 8.3 Font System

- `kitty/fonts.c` — The font-group and cell-metric engine (1761 lines). Key functions: `load_fonts_data()`, `font_group_for()` (line ~204), `initialize_font_group()` (line ~1495), `calc_cell_metrics()` (line ~373), `set_font_data()`, `sprite_tracker_set_layout()` (lines ~276–290), `sprite_tracker_set_limits()`, `send_prerendered_sprites()`.
- `kitty/fonts.h` — Font data-type declarations including `FontGroup`, sprite tracker, and shaper handles.
- `kitty/freetype.c` — FreeType integration (1037 lines). Key functions: `cell_metrics()` (lines ~387–405), `calc_cell_width()` (lines ~374–384), `calc_cell_height()` (lines ~141–154), `face_from_descriptor()` (line ~262), and variable-font-axis handling.
- `kitty/fontconfig.c` — Linux font discovery via `dlopen()`-based loading of FontConfig. `load_fontconfig_lib()` (line ~79) tries multiple candidate library names — `"libfontconfig.so"` first, then `"libfontconfig.so.1"` as a fallback — each with `RTLD_LAZY`. Implements Python bindings for `fc_list`, `fc_match`, and `fc_match_postscript_name`, plus pattern-to-dict conversion that exposes family/style/slant/weight/file/index.
- `kitty/fonts/render.py` — The Python face of the font stack (531 lines). Key functions: `set_font_family()` (line ~173), `dump_font_debug()` (lines ~161–171), `descriptor_for_idx()` (line ~157).
...
- `glfw/glfw3.h` — Public API header for GLFW 3.4, including the `GLFW_CONTEXT_VERSION_MAJOR`, `GLFW_CONTEXT_VERSION_MINOR`, `GLFW_OPENGL_FORWARD_COMPAT`, `GLFW_DEPTH_BITS`, `GLFW_STENCIL_BITS`, and `GLFW_SRGB_CAPABLE` hint constants actually used by `kitty/glfw.c:create_os_window()` (see Section 2 for the enumeration). The header also defines `GLFW_OPENGL_PROFILE`, but that symbol is only referenced by `kitty/glfw.c` at line 2492 via `ADDC(GLFW_OPENGL_PROFILE)` — a Python-binding constant registration — not via any `glfwWindowHint()` call in `create_os_window()`.
...
- `go.mod`, `go.sum` — Go module definition and dependency graph for the CLI tooling (Go 1.22). Direct dependencies include `github.com/ALTree/bigfloat` (arbitrary-precision float), `github.com/alecthomas/chroma/v2` (syntax highlighting, v2 major version), `github.com/bmatcuk/doublestar/v4` (glob, v4 major version), `github.com/kovidgoyal/imaging` (the direct imaging dependency actually used by kitty's Go tooling — a fork of the upstream `disintegration/imaging`), `github.com/shirou/gopsutil/v3` (v3 major version), and `github.com/zeebo/xxh3`. Indirect dependencies include `github.com/disintegration/imaging` (pulled in transitively, marked `// indirect` in `go.mod`) and `github.com/klauspost/cpuid/v2` (v2 major version, indirect). Go requires the `/v2`, `/v3`, `/v4` suffixes in the import path for any major version ≥ 2 (the "SIV" semantic-import-versioning rule), so the suffixes are part of the canonical module path.
...
- `./kitty/launcher/kitty --debug-font-fallback` — enables a flag that causes `kitty/fonts/render.py:dump_font_debug()` (lines ~161–171) to execute after font-family init (post Step 11). It prints the full descriptor set (medium, bold, italic, bold-italic, symbol_map entries) and, for each resolved font, lists the path and face index.
...
- `compositor_name()` (lines ~206–230) — detects X11 vs Wayland via environment variables, identifies the window-system compositor by inspecting process trees and well-known socket names, with a special case for Hyprland's environment-variable conventions.
...
| Font family and descriptors | `kitty/fonts/render.py`, `kitty/fontconfig.c` | `set_font_family()` (~173), `dump_font_debug()` (~161–171), `fc_match` bindings |
| Cell metrics (pixel sizing) | `kitty/freetype.c`, `kitty/fonts.c` | `cell_metrics()` (~387–405), `calc_cell_width()` (~374–384), `calc_cell_height()` (~141–154), `calc_cell_metrics()` (~373) |
