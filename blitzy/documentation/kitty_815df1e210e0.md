...
The cwd hint uses the same in-order dispatch: `case 7:` `[kitty/vt-parser.c:L499]` calls `process_cwd_notification(self->screen, code, ...)` `[kitty/vt-parser.c:L505]`. In `screen.c`, `process_cwd_notification` `[kitty/screen.c:L2393]` handles `if (code == 7)` `[kitty/screen.c:L2394]` by storing the reported path into `last_reported_cwd` `[kitty/screen.c:L2397-L2398]`. What the source guarantees here is precise but modest: because OSC 7 flows through the same byte-ordered parser as the surrounding text, each OSC 7 simply **updates Kitty's latest reported cwd in byte order** — `last_reported_cwd` always reflects the most recent OSC 7 seen in the stream, never a stale or racing value. *(The cited code stores only this single latest value; it does not, on its own, bind a cwd to a particular prompt — any prompt-to-cwd association would be layered on top elsewhere.)*
...
> **Industry terminology check:** The synchronized-output spec is set with `CSI ? 2026 h` and reset with `CSI ? 2026 l`; its stated goal is to avoid showing a half-drawn screen by deferring rendering until the update completes (or a safety timeout fires). Reference implementations adopt a bounded timeout — for example, tmux defers flushing until the application disables sync or a ~1-second timeout expires. **Kitty implements *both* forms:** the standard CSI `?2026` pending-mode set/reset path *and* an alternate **DCS** form of the same idea (`ESC P =1s ESC \` to begin, `ESC P =2s ESC \` to end). Both converge on the same `screen_pause_rendering` with a **2000 ms** safety timeout (see e.1).
...
| Input encode + callbacks | `kitty/glfw.c:L430,L330,L115-L116`; `kitty/keys.c:L311`; `kitty/mouse.c:L112` (`encode_mouse_event`) |
| Screen state + markers | `kitty/screen.c:L2316,L2328-L2352` (OSC 133), `L2393` (OSC 7) |
| Pause/resume (sync output) | CSI `?2026`: `kitty/control-codes.h:L235`, `kitty/modes.h:L86`, `kitty/vt-parser.c:L997,L1006,L1010`, `kitty/screen.c:L1101,L1174-L1175`; DCS `=1s/=2s`: `kitty/vt-parser.c:L620,L640,L645`; snapshot/timeout: `kitty/screen.c:L2506,L2521,L2489,L2738` |
