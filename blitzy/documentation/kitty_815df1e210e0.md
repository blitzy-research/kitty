...
> from another terminal against a running kitty instance. Every behavioral claim below is either
> **(observed)** — captured from a real build-and-run — or **(inferred from code)** — read from
> the source with a `file:line` citation. Byte-sensitive artifacts (the DCS envelope) are shown as
> the *literal bytes emitted*, with `ESC` rendered as `0x1b`.
>
...
1. **How does the kitten know *where* to send the command — socket, pipe, or something else?**
   ...
   a socket (`unix:…`, `tcp:…`, or `fd:…`) it connects to that socket; **if it is empty it writes
   DCS escape codes to the controlling TTY** — the window's own pty. The Go client picks this at
   `tools/cmd/at/main.go:371-376`; the Python client at `kitty/remote_control.py:383`
...
2. **What is the real socket path, and why did `/tmp` look empty?**
   **Because in the default configuration kitty creates no socket at all** — that is why `/tmp`
   looked empty. A socket is opened only inside `Boss.listen_on()` (`kitty/boss.py:177`) and only
...
`tools/cmd/at/main.go:371-376` — if `--to` was not supplied, fall back to the env var, then parse it
...
`os.NewFile(uintptr(fd), ...)`), not by `ParseSocketAddress`.
...
KITTY_PID=26964
KITTY_PUBLIC_KEY=1:<REDACTED_EPHEMERAL_PER_INSTANCE_PUBLIC_KEY>
...
source of the confusion addressed in §4.
...
The default `allow_remote_control` is `no` (`kitty/options/definition.py:479`), so even if you passed
`--listen-on`, the default level would *still* skip socket creation. The actual bind/listen happens
in `Boss.listen_on()` (`kitty/boss.py:177`), and the socket file is removed at exit via
...
json.dumps(response))` — since `ESC_DCS` frames as `ESC P … ESC \`, both transports emit the same
shape.

Raw response captured straight off the socket (3281 bytes total), `hexdump -C`:
...
- Same 12-byte prefix `ESC P @kitty-cmd`, then `{"ok": true, "data": "[…]"}`, then the 2-byte
...
- The C event loop delivers the socket bytes to Python via `PyObject_CallMethod(global_state.boss,
  "peer_message_received", "y#KO", ...)` (`kitty/child-monitor.c:504`). The bytes arriving at
...
```
[ 25.197] TRACE window.Window.handle_remote_cmd window=1 cmd=b'{"cmd":"ls","version":[0,26,0],"kitty_window_id"...'
[ 25.197] TRACE boss.handle_remote_cmd         window=1 cmd=b'{"cmd":"ls","version":[0,26,0],"kitty_window_id"...'
[ 25.198] TRACE boss._handle_remote_command    peer_id=0 window=1 cmd=b'{"cmd":"ls","version":[0,26,0],"kitty_window_id"...'
[ 25.198] TRACE remote_control.parse_cmd       serialized=b'{"cmd":"ls","version":[0,26,0],"kitty_window_id"...'
...
- The DCS reaches Python through the VT parser: `kitty/vt-parser.c:603` `dispatch("cmd{",
  ...
  object *is* the `Window` (`kitty/window.py:604` `Screen(self, ...)`), so this lands on
...
$ printf '\033P@kitty-cmd{"cmd":"ls","version":[0,14,2]}\033\\' | socat - unix:/tmp/test \
    | awk '{ print substr($0, 13, length($0) - 14) }' | head -c 400
...
Below is the **complete, unedited** 110-line tree from a single-OS-window headless instance. The
only edit is the redaction of the ephemeral per-instance `KITTY_PUBLIC_KEY` value (see the note after
the block); every other byte is as emitted.
        ...
        "windows": [
          {
            ...
            "columns": 71,
            "created_at": 1783486347085216591,
            "cwd": "/app",
            "env": {
              "COLORTERM": "truecolor",
              "DISPLAY": ":99",
...
> **Redaction note (rule compliance).** `KITTY_PUBLIC_KEY` is an **ephemeral per-instance** X25519
> public key regenerated on every launch (`kitty/child.py:245`); it is not a durable credential. Its
> value is replaced above with `1:<REDACTED_EPHEMERAL_PER_INSTANCE_PUBLIC_KEY>`; the real capture
> contained a 40-odd-character Base-85 string after `1:`.
...
`{"ok": true, "data": …}` envelope, wire-framed as `0x1b P @kitty-cmd <json> 0x1b \`. If you return a
plain object it is embedded directly; `ls` chooses to pre-serialize its tree with
`json.dumps(..., indent=2, sort_keys=True)`, which is why its `data` is a *string*. See §11 for the
framework details.
...
only", and the version-mismatch message — are returned as `{"ok": false, "error": …}` and produce
...
`handle_cmd` returns `None` when `no_response` is set (`kitty/remote_control.py:262-263`), and the
client's `do_io` returns `{'ok': True}` immediately without reading a reply
(`kitty/remote_control.py:386-387`). Note the Go `kitten @` CLI does **not** expose this as a flag for
`ls`:
...
Go: `tools/cmd/at/socket_io.go:170` `os.NewFile(uintptr(fd), ...)`. It is **not** handled by
...
- `response_from_kitty(self, boss, window, payload_get)` (`kitty/rc/base.py:418`) — runs in the
  **kitty process**; computes the result and returns it. Its return value becomes `response['data']`
  in the `{"ok": true, "data": …}` envelope (§7).
...
# default (TTY) config:
./kitty/launcher/kitty --start-as=minimized -o allow_remote_control=yes <cmd> &
# explicit socket config (canonical docs example):
./kitty/launcher/kitty --start-as=minimized -o allow_remote_control=socket-only --listen-on unix:/tmp/test <cmd> &
```
...
| Q4 | What the protocol messages look like on the wire | DCS envelope `0x1b P @kitty-cmd <json> 0x1b \`. | §1, §5 | `hexdump -C` of request (45 B) and raw response (3281 B); `encode_send` / `encode_response_for_peer` |
...
| `allow_remote_control` gate | §3, §9 | `kitty/boss.py:356-369` (socket gate `:364`); default `no` `options/definition.py:479` |
| `listen_on()` socket creation | §3 | `kitty/boss.py:177-188` |
...
| shell-integration env vars (`KITTY_PID`/`KITTY_WINDOW_ID`) | §4 | `shell-integration/zsh/kitty-integration:249,251`; `bash/kitty.bash:215-216` |
...
| C bridge `PyObject_CallMethod(... "peer_message_received" ...)` | §6 | `kitty/child-monitor.c:504` |
| `Window.handle_remote_cmd` → `Boss.handle_remote_cmd` (TTY entry) | §6 | `kitty/window.py:1279-1280`; `kitty/boss.py:849-852` |
| VT-parser DCS dispatch | §6 | `kitty/vt-parser.c:600,603`; `screen.c:87-91` CALLBACK |
...
| **Edge:** `--no-response` | §9 | protocol `no_response:true` → 0-byte reply (`remote_control.py:262-263`); not a CLI flag |
...
- **Observed vs inferred** labeled throughout; SSH round-trip is the only inferred-from-code item.
