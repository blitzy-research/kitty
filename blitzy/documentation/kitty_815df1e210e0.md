...
Clean rebuild completed successfully (exit 0), compiling every C source with strict `-Werror`. The **complete, unedited** tail of the build output (the final link steps and the Go-generate step) is reproduced below verbatim — it proves the CPython extension and native launcher were produced:
...
kitty/tools/cmd
/usr/bin/go build -v -ldflags '-X kitty.VCSRevision=a75a0adb16953016ea8e71b426a5ae8fa40482d1 -s -w' -o kitty/launcher/kitten /tmp/blitzy/kitty/blitzy-9f9917e5-4911-4b94-9f22-1256c050c290_c8ba5a/tools/cmd
```

> **Build-time HEAD note (grounded).** This investigation targets branch `kitty_815df1e210e0` at its baseline commit `815df1e210e0a9ab4622f5c7f2d6891d7dbeddf1` (the branch after which the deliverable is named). The build above ran with the working tree at the branch HEAD `a75a0adb16953016ea8e71b426a5ae8fa40482d1` — the commit that adds *this* document on top of that baseline. Consequently the Go linker embeds `-X kitty.VCSRevision=a75a0adb16953016ea8e71b426a5ae8fa40482d1` (final line above) and the startup banner (see §Q1.2) reports `kitty 0.35.2 (a75a0adb16)` via `version(add_rev=True)` (`kitty/cli.py:L490`; `rev = KITTY_VCS_REV[:10]`). Plain `--version` omits the revision.

```
$ ls -l ./kitty/launcher/kitty
-rwxr-xr-x 1 root root 40384 Jul  8 05:32 ./kitty/launcher/kitty
...
```
kitty 0.35.2 (a75a0adb16) created by Kovid Goyal
...
This supplies every AAP-predicted field: the version line **with VCS revision** `kitty 0.35.2 (a75a0adb16)` (`version(add_rev=True)`, `kitty/cli.py:L490`); the OS/kernel `uname` line; `Running under: X11`; `OpenGL: '4.5 (Core Profile) Mesa 25.2.8-0ubuntu0.25.10.2' Detected version: 4.5` (`opengl_version_string`, `kitty/glfw.c:L1426`, identical to the §Q1.2 GL line); `Frozen: False`; and the `Fonts:` block with the `medium/bold/italic/bi` labels — the same four DejaVu Sans Mono faces the `--debug-font-fallback` `Text fonts:` block reports in §Q1.2, under kitty's alternate label set.
...
1. **GL-level check (enabled by `--debug-rendering`).** `check_for_gl_error` (`kitty/gl.c:L16`) is installed as a GLAD post-callback (`kitty/gl.c:L62`) and calls `fatal()` with a message beginning `OpenGL error:` (`kitty/gl.c:L17`) after *any* GL call that errors. This callback is live **precisely because** we pass `--debug-rendering`: on a non-debug run the preceding `gladUninstallGLDebug()` (`kitty/gl.c:L59`) removes it. Consequently every GL call in the atlas path — the `glTexStorage3D` in `realloc_sprite_texture` and the `glTexSubImage3D` in `send_sprite_to_gpu` — is error-checked at startup, and any failure would terminate the process with a fatal OpenGL error rather than continue.
