...
Range **4–8**, mode **4** (35/40 ≈ 88 %). Aggregated over all 40 runs: `children count` at divergence ∈ `{15 ×4, 16 ×40, 17 ×55, 18 ×73}` (always `< 20`); `add queue` = `0` at every one of the **172** samples; stale ids seen = `{2 ×160, 3 ×9, 4 ×1, 5 ×2}`; exceptions = `0`. **The diagnostic count is therefore _not_ deterministic — it is the distribution tabulated above.**

The one property that is **mechanism-guaranteed** — timing-independent — is the **safe reconciliation itself**: in every run every child is reaped and **zero** `KeyError`/tracebacks occur (this is the same invariant flagged at *"exceptions … invariant, not the count"* above). The remaining envelope numbers — `children count < 20` and `add queue == 0` — are **observations that held across these canonical runs, not hard guarantees**. An **independent re-validation** (the same hashed input `19cfee89…`, 40 fresh canonical `--debug-rendering` runs on this container, no shim) reproduced the **mode 4** count and the whole envelope — `add queue` `= 0` at all **160** samples, `children count` ≤ 18, and `0` exceptions — confirming stability per the run-first ≥2-run requirement. To show those two envelope values are *timing-dependent* rather than truly invariant, a deliberately **non-canonical** stress (pinning kitty **and** CPU-burners to a single core to starve the scheduler — *not* the default run) pushes the diagnostic count well past 8 (observed into the dozens) and leaves the add-queue transiently **non-empty** at some *id-in-neither* diagnostics (`add queue: 1` observed in **61 of 920** stress samples, e.g. `… child with id: 4 (children count: 5) (add queue: 1)`) — **yet still with zero exceptions and every child reaped**. So the count, the specific stale ids, the `children count` bound, and even `add queue == 0` are precisely the run-to-run variation the question asks about (reported rather than smoothed away); the crash-free, all-children-reaped reconciliation is the guarantee. *(The stress run only makes the add-queue non-empty at an `id-in-neither` diagnostic; it does **not** reproduce the positive add-queue-**hit** branch of Q2, which remains labeled inferred there.)*
