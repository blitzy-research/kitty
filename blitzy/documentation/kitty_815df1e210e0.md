...
- **Capacity note:** the burst fills five segments — the initial segment is allocated at construction (`add_segment` inside `create_historybuf`, `kitty/history.c:L127`), and the burst then crosses four 2048-line allocation boundaries (at pushes #2048/#4096/#6144/#8192) — only because `ynum=10240` was chosen well above the default. A **default-configured** window uses `scrollback_lines = 2000` (`kitty/options/definition.py:L372`), and `2000 < 2048 = SEGMENT_SIZE`, so a default window's entire scrollback fits inside a **single segment** — segment carving is *only observable when scrollback exceeds the default*. This is why the harness uses `ynum=10240`.
...
**A separate, heavier transition the push-only burst did *not* trigger — pager rewrap/reflow.** Everything measured above concerns *segment-boundary pushes*, which never reflow or move already-stored lines: `add_segment` simply appends a fresh segment and leaves the existing segments untouched. A genuinely different transition exists on a **column resize**, though — the pager ring's stored bytes must be re-wrapped to the new width by the separate `pagerhist_rewrap_to` path (`kitty/history.c:L391-L432`). That routine does *not* edit the ring in place: it `calloc`s a brand-new `PagerHistoryBuf` and a fresh ring (`kitty/history.c:L395`, `kitty/history.c:L398`), walks the entire existing ring character-by-character and re-emits each line wrapped to the new `cells_in_line` (`kitty/history.c:L417-L428`), then frees the old ring and swaps in the new one (`kitty/history.c:L429-L430`). It is thus a full O(bytes) rebuild of the pager rather than an O(1) appended segment, so a width change is a distinctly costlier transition than the segment carving timed above. **Flagged as verified-by-reading, NOT by running:** the push-only burst harness never resized the buffer, so `pagerhist_rewrap_to` was not exercised at runtime in these observations; its behavior is stated from the source only.
...
1. **The ringbuf backing store lives at the repository root: `3rdparty/ringbuf/ringbuf.h` — not under the `kitty/` subtree.** Proof: `kitty/history.c:L12` includes it as `#include "../3rdparty/ringbuf/ringbuf.h"` — the `../` climbs out of `kitty/` to the repository root. Confirmed on disk: there is no nested `3rdparty` directory beneath `kitty/`, whereas the repository-root `3rdparty/ringbuf/ringbuf.h` does exist (252 lines). All pager-ring citations here therefore use `3rdparty/ringbuf/ringbuf.h`.
...
| **OBJ-3** — smooth, or subtle hesitations? | [OBJ-3](#obj-3--does-it-transition-smoothly-or-hesitate) | Mostly smooth; predictable ~4–7× median hesitation at each 2048 boundary (realloc+calloc in `add_segment`); rarer large off-boundary jitter; fatal-on-OOM (`kitty/history.c:L21,L26`); the separate pager rewrap/reflow on resize (`pagerhist_rewrap_to`, `kitty/history.c:L391-L432`) is a full O(bytes) rebuild, flagged verified-by-reading |
...
- **OBJ-3 pager rewrap/reflow (`pagerhist_rewrap_to`) was NOT exercised.** The separate width-resize reflow path at `kitty/history.c:L391-L432` — which `calloc`s a new pager ring and re-wraps every stored line to the new column count (`kitty/history.c:L417-L428`), then frees the old ring and swaps in the new one (`kitty/history.c:L429-L430`) — is confirmed **by reading the code only**. The push-only burst harness never triggered a column resize, so this rebuild was not observed at runtime here.
