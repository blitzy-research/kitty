...
Run with `--verbose` it additionally prints, in order: compiler detection, the **two gcc commands** (one compile line for `data-types.c`, then one link line assembling ~60 object files into `fast_data_types.so`), and finally the Go build of the `kitten` client. The block below is a single `--verbose` run captured live during this investigation (forced by `touch`-ing one C source so the compile is re-triggered); it is shown complete, with **both** gcc commands verbatim and unelided. The compile line's `-DKITTY_VCS_REV=` and the Go line's `VCSRevision=` both carry the identical 40‑hex commit that was checked out when this capture ran — a **volatile** value that tracks the built checkout (see the note after the block):
...
/usr/local/go/bin/go build -v -ldflags '-X kitty.VCSRevision=6865674c70db7db08f2ca81fd62b2a388a50b569 -s -w' -o kitty/launcher/kitten /tmp/blitzy/kitty/blitzy-5272820a-f82d-4851-9076-f4ffb122d22c_31872f/tools/cmd
```

(The 40‑hex string in the block above — `6865674c70db7db08f2ca81fd62b2a388a50b569`, visible identically in the gcc compile line's `-DKITTY_VCS_REV=` and the Go line's `VCSRevision=` — is a **volatile** value, exactly like the PIDs, socket inodes, timestamps, and ephemeral public key reported elsewhere in this document. `setup.py` derives it from `git rev-parse HEAD` at build time (`setup.py:674-678`), emitting it both as the C compile-time define `-DKITTY_VCS_REV=` (`setup.py:726`) and the Go link-time flag `-X kitty.VCSRevision=` (`setup.py:1149-1151`) — which is why the two stamps are identical: they are the same `HEAD`. Because the stamp is simply whatever commit is checked out, it is **not** an invariant to reproduce: this deliverable is delivered by documentation-only commit(s) that each advance the branch *after* a capture is taken, and you may build from any checkout, so a fresh `python3 setup.py` on your tree will embed *your* current `HEAD` — a different 40‑hex string — and that is correct, not stale. (The **initial** documentation commit — the first commit that added this file — was `7da6b37726335799f63f5ea0bca9185625382c21`, a fixed historical fact also recorded in the header and the coverage ledger, §9.) None of the documentation commits touches any C, Go, or Python file, so the compiled source is byte-identical to `815df1e2` and the binary's behavior is unchanged — as explained in the header. One further run-specific detail: `go build -v` prints the name of each package it *compiles* (e.g. `kitty/tools/cmd`); because the `VCSRevision` travels through a link-time `-ldflags -X` and not a source change, a warm Go build cache re-links the client without recompiling that package, so — as in the capture above — no package line is printed (on a cold cache Go prints one or more such lines; either way the `VCSRevision=` stamp on the build command is identical).
...
|---|-------|--------|----------|
| 1 | Build is `python3 setup.py`; launcher at `kitty/launcher/kitty`; banner `kitty 0.35.2`; binary stamps `VCSRevision` with the checked-out git HEAD — a **volatile** value (the §2 live capture shows `6865674c…`; a build on your checkout stamps your own HEAD); the initial documentation commit was `7da6b377` | OBSERVED | §2 `--verbose` build tail + `--version` |
...
*End of document. All runtime evidence above was captured on `kitty 0.35.2` (source tree at base VCS `815df1e210e0a9ab4622f5c7f2d6891d7dbeddf1`; the built binary stamps `VCSRevision` with the git HEAD checked out at build time — a volatile value, see §2 — and the initial documentation commit was `7da6b377`) inside the prepared container, under `Xvfb :99` with the Mesa `llvmpipe` software GL renderer. Volatile values (the build's `VCSRevision` stamp — the checked‑out git `HEAD`, e.g. `6865674c…` in §2; PIDs such as `40579`/`42106`/`42182` in §4‑§5 and `41386`/`41484`/`41580` in §6; socket inode numbers, timestamps, and the ephemeral public key) are reported exactly as observed and will differ on other runs.*
