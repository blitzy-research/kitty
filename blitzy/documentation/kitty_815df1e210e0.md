...
By contrast, the two sibling actions do **not** persist:
...
- **Grounding:** every factual claim cites a specific `file:line` and/or shows captured output. The few statements that are reasoned from code rather than directly observed are explicitly labeled **(inferred, not observed)** — in this investigation, all four final-pane actions and all three reload variants were *directly observed*, so no reload behavior needed that label.

---

## 3. Q1 — Build & Default Launch

**Direct answer:** kitty is built from this checkout with the canonical developer command **`./dev.sh build`**, which produces the in-place launchers `kitty/launcher/kitty` and `kitty/launcher/kitten`. Launching `kitty/launcher/kitty --version` prints the observed banner **`kitty 0.35.2 created by Kovid Goyal`**.

### Toolchain (prerequisites for the build)
- **Go** — `go version go1.22.12 linux/amd64`, satisfying the `go 1.22` requirement declared at `go.mod:L3`.
- **C compiler** — `gcc (Ubuntu 15.2.0-4ubuntu4) 15.2.0` (a C compiler + the Go compiler are the two hard requirements per `docs/build.rst:L14-L16`).
- **Native font/runtime libraries** (via `pkg-config`, per `docs/build.rst`): harfbuzz 10.2.0, freetype2 26.2.20, fontconfig 2.15.0, libpng 1.6.50, lcms2 2.16, zlib 1.3.1, libcanberra 0.30.
...
Driven inside a headless kitty GUI, `kitten choose-fonts` renders its first screen — the **family-list pane** — with fonts enumerated by the Python backend:

```
>DejaVu Sans Mono      ║            DejaVu Sans Mono
 Fira Code             ║
 Inconsolata           ║ Styles: Bold, Bold Oblique, Book,
 JetBrains Mono        ║ Oblique
 JetBrains Mono NL     ║
 Liberation Mono       ║ Press the Enter key to choose this
 Noto Mono             ║ family
...
Both names resolve at runtime — they appear in the `kitten` command list, and both accept `--help`:
```
...
```
$ kitty/launcher/kitten choose-fonts --reload-in=bogus </dev/null ; echo "exit=$?"
...
1. **Family-list pane** (`list.go` / `family_list.go`). Incremental filtering is handled by `family_list.go:L103` (`UpdateFamilies`); the current family is `family_list.go:L155-L157` (`CurrentFamily`). On **Enter**, `list.go:L246-L250` transfers control via `self.handler.faces.on_enter(family)`.
   ...
   >Fira Code ║                      Fira Code
              ║
              ║ Styles: Bold, Light, Medium, Regular, Retina,
              ║ SemiBold
              ║
...
The four keys that get written are produced by `serialized()` (`final.go:L63-L70`), which joins the four settings with **fixed column padding** (each key is padded to column 17, value begins at column 18) and **no trailing newline**:
```
font_family      <spec>
bold_font        <spec>
italic_font      <spec>
...
The Enter handler (`kittens/choose_fonts/final.go:L78-L95`):
```
...
- write the file **atomically** via `utils.AtomicUpdateFile` (`L347`), which is a temp-write + `os.Rename` that preserves the existing file's permissions (`tools/utils/atomic-write.go:L79-L88`).
...
The reload is delivered by `ReloadConfigInKitty` (`tools/config/api.go:L352-L371`), a **`SIGUSR1`** signal — to the parent process via `KITTY_PID` when `in_parent_only` is true (`L354-L357`), or to *all* kitty-GUI processes otherwise (`L363-L366`). This is a signal-based mechanism, distinct from the `load_config` remote-control command.
...
```
$ ls -la /tmp/kcfg1.BJCLKU
total 8
drwx--S---  2 root root 4096 Jul  6 22:19 .
drwxrwsrwx 11 root root 4096 Jul  6 22:19 ..
$ cat /tmp/kcfg1.BJCLKU/kitty.conf
cat: /tmp/kcfg1.BJCLKU/kitty.conf: No such file or directory (os error 2)
(kitty.conf ABSENT)
```

Drive: filter `Fira` → Enter (choose family) → Enter (accept previews) → **Enter (finalize)**.

**AFTER** — `kitty.conf` now exists (139 bytes). Shown with `cat -A` (line ends as `$`, spaces literal) to prove the exact padding and that there is **no trailing newline** after `# END_KITTY_FONTS` (matching the `api.go:L330` addition format):
```
$ cat -A /tmp/kcfg1.BJCLKU/kitty.conf
# BEGIN_KITTY_FONTS$
...
# END_KITTY_FONTS
```
No `.bak` was produced here — as predicted by the `len(raw) > 0` guard (`api.go:L343`): the prior file was absent/empty, so there was nothing to back up.
...
>Fira Code             ║
 Inconsolata           ║ Styles: Bold, Light, Medium, Regular,
 JetBrains Mono        ║ Retina, SemiBold
```
...
```
$ KITTY_CONFIG_DIRECTORY=/tmp/kcfg1.BJCLKU kitty/launcher/kitty +runpy \
    'import sys; from kitty.constants import defconf; from kitty.config import load_config; \
     o=load_config(defconf); sys.stdout.write("config file read: %s\n" % defconf); \
     sys.stdout.write("RESOLVED font_family = %r\n" % (o.font_family,))'
config file read: /tmp/kcfg1.BJCLKU/kitty.conf
...
```
--- run2_before_kitty.conf
+++ run2_after_kitty.conf
@@ -1,4 +1,12 @@
...
```
The prior `font_family      JetBrains Mono` became `# font_family      JetBrains Mono` (comment-out regex, `api.go:L325-L326`); `font_size` and `cursor_shape` were preserved (they are not in the settings-to-comment-out list); the block was appended after two blank lines (`api.go:L337`).

The `.bak` backup was created (guarded path `api.go:L343-L344`) and contains the verbatim original:
```
$ cat /tmp/kcfg2.ZfZOcr/kitty.conf.bak
# My existing kitty configuration
font_family      JetBrains Mono
font_size 11
cursor_shape block
...
kitty core re-reads the updated file and resolves Fira Code (the commented-out JetBrains Mono is inert, the block wins):
```
$ KITTY_CONFIG_DIRECTORY=/tmp/kcfg2.ZfZOcr kitty/launcher/kitty +runpy '...load_config(defconf)...'
config file read: /tmp/kcfg2.ZfZOcr/kitty.conf
RESOLVED font_family = FontSpec(family='Fira Code', ..., created_from_string='family="Fira Code"')
```

### 6.4 Contrast: the non-persisting siblings

- **`s` (STDOUT only).** From the final pane, `s`/`S` sets `output_on_exit = self.settings.serialized() + "\n"` and quits (`final.go:L101-L108`); `main.go:L64-L66` writes `output_on_exit` to STDOUT on exit. Observed STDOUT (note: it is the raw `serialized()` — the four keys **without** the `# BEGIN/END_KITTY_FONTS` sentinels that only the Enter/`Patcher` path adds):
  ```
  font_family      family="Fira Code"
  bold_font        auto
  italic_font      auto
  bold_italic_font auto
  ```
  The config dir's `kitty.conf` remained **absent** after `s` — confirming it does not persist.
- **`Esc` (abort).** From the final pane, `Esc` sets `current_pane = faces` and returns without writing (`final.go:L72-L77`). Observed: the screen returned to the faces/previews pane and `kitty.conf` stayed absent.
- **`Ctrl+c` (quit).** Handled by the loop (`tools/tui/loop/run.go:L151-L153` → `on_SIGINT` → quit); observed the kitten exited with no `kitty.conf` written.
...
|---|---|---|---|---|
| **Enter**, `--reload-in parent` (default) | Writes `# BEGIN_KITTY_FONTS` block; sends `SIGUSR1` to parent (`KITTY_PID`) | **Yes** | Yes — parent only | `final.go:L78-L89`; `api.go:L352-L357`; §6.2, `sig_parent.log="GOT_SIGUSR1"` |
| **Enter**, `--reload-in all` | Writes block; sends `SIGUSR1` to all kitty GUIs | **Yes** | Yes — all instances | `final.go:L90-L91`; `api.go:L363-L366`; §7 note, `sig_all.log="GOT_SIGUSR1"` |
| **Enter**, `--reload-in none` | Writes block; **no** reload signal (no `switch` case) | **Yes** | No | `final.go:L87-L92` (no `none` case); `sig_none.log` empty |
| **`s` / `S`** | Serialized settings to **STDOUT**; nothing written to disk | No | No | `final.go:L101-L108`, `main.go:L64-L66`; §6.4 |
| **`Esc`** | Abort → return to faces pane; nothing written | No | No | `final.go:L72-L77`; §6.4 |
| **`Ctrl+c`** | Quit; nothing written | No | No | `final.go:L44`, loop `run.go:L151-L153`; §6.4 |

**Directly observed reload behavior** (a fake "kitty" GUI stand-in that traps `SIGUSR1` was pointed at by `KITTY_PID`):

| `--reload-in` | `kitty.conf` written? | `SIGUSR1` delivered to parent? |
|---|---|---|
| `parent` | yes | **YES** (`GOT_SIGUSR1`) |
| `all` | yes | **YES** (`GOT_SIGUSR1`) |
| `none` | yes | no (log empty) |

The write (persistence) is independent of `--reload-in`; only the live reload signal differs.
...
flowchart TD
    ...
    G -->|"none (no case)"| J["no reload signal<br/>final.go:L87-92"]
    ...
    E -->|"Ctrl+c<br/>run.go:L151-153"| M["quit, no write"]
```
...
# AFTER Enter (cat -A shows line-ends as $)
$ cat -A /tmp/kcfg1.BJCLKU/kitty.conf
# BEGIN_KITTY_FONTS$
font_family      family="Fira Code"$
bold_font        auto$
...
```
$ diff -u run2_before_kitty.conf run2_after_kitty.conf
@@ -1,4 +1,12 @@
 # My existing kitty configuration
-font_family      JetBrains Mono
+# font_family      JetBrains Mono
...
### 9.6 Sibling conditions — `s` STDOUT (Q4 contrast)
```
# kitten STDOUT captured from the `s` action (raw serialized(), NO sentinels):
font_family      family="Fira Code"
bold_font        auto
italic_font      auto
bold_italic_font auto
# and the config dir's kitty.conf remained ABSENT (not persisted)
```

### 9.7 Reload variants — directly observed `SIGUSR1` (Q3d / condition coverage)
```
reload-in=parent   kitty.conf written=yes   SIGUSR1 to fake parent = GOT_SIGUSR1
reload-in=all      kitty.conf written=yes   SIGUSR1 to fake parent = GOT_SIGUSR1
reload-in=none     kitty.conf written=yes   SIGUSR1 to fake parent = (none)
```
(The stand-in that received the signal had `argv[0]` basename `kitty`, satisfying `is_kitty_gui_cmdline` at `tools/config/api.go:L282-L303`; `KITTY_PID` was exported to point at it before exec'ing the kitten. The `kitty.conf` written by each variant was byte-identical to Run 1's block.)

### 9.8 Repository cleanliness proof (`git status`)
Captured at the repository root **after** deleting every temporary observation script (`/tmp/drive_choose_fonts.sh`, `/tmp/fake_kitty.sh`) and every scratch config directory (`/tmp/kcfg*`), removing the sockets/logs/evidence dir, and stopping the spawned `Xvfb`/kitty GUI processes:
...
$ git diff --cached --name-only   # staged changes
                                  # (no output — nothing staged before commit)
```

The working tree shows **exactly one** untracked path — this document. No tracked source, docs, config, tests, build, or CI file is modified or deleted; the `blitzy/screen_recordings/` and `blitzy/screenshots/` directories are pre-existing empty scaffolding (git does not report empty directories). The repository is byte-for-byte unchanged apart from the single new answer document, satisfying the read-only scope rule.

---

## 10. Coverage Checklist

- **Q1 — Build & default launch:** §3 + §9.1. Command `./dev.sh build` (`dev.sh:L9`); binaries `kitty/launcher/{kitty,kitten}`; observed banner `kitty 0.35.2 created by Kovid Goyal`.
...
- **Q3a — Registration:** §5(a) + §9.2. `tools/cmd/tool/main.go:L9,L82`; visible clone alias `choose_fonts` (`main.go:L96-L98`); both names resolve.
...
- **Q4 — Persistence (LEAD):** §1 + §6 + §9.4–9.7. Before/after/restart proven at kitten and kitty-core level; `Patcher.Patch` mechanism; `.bak` and comment-out observed; `s`/`Esc`/`Ctrl+c` non-persistence contrasted; `--config NONE` caveat.
- **Condition coverage:** §7. Enter (+`--reload-in parent`/`all`/`none`), `s`/`S`, `Esc`, `Ctrl+c` — each with `file:line` + captured output. All reload variants **directly observed**.
