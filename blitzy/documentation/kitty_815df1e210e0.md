...
**Observed evidence.** With `--debug-input`, pressing `l`, `s`, Return produced these **consecutive, unfiltered** STDERR lines (the harness prints them straight from the raw error log, so both the platform XKB line and the Kitty line are visible in order; verbatim `cat -v`):
