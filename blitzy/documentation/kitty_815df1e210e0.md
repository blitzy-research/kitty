...
- **Restricted remote control.** Where remote control is used as a read-back harness it listens on a socket **inside** that private `0700` directory, and kitty is started with `allow_remote_control=socket-only`. This value gates purely by **transport**, not by whether the caller runs inside the terminal: requests arriving over a socket are accepted *unconditionally*, while requests arriving over the TTY (terminal escape sequences) are denied `[kitty/options/definition.py:2982-2985; kitty/boss.py:600-601,623-632]`. It therefore does **not** stop a program *inside* the terminal from controlling kitty: a same-UID child inherits the socket address as `KITTY_LISTEN_ON` `[kitty/child.py:246-247]`, which `kitty @` uses automatically when no `--to` is given `[kitty/remote_control.py:270-273]`. Observed with the real binary — from such a child, `kitty @ ls` (no `--to`) exits `0` and returns the full window/session JSON over the socket (2855 bytes in this run; the exact size varies with live state), while the escape-sequence (TTY) path returns no session data. The boundary this harness actually relies on is the enclosing **`0700` directory** (observed mode `drwx------`): ordinary filesystem permissions keep the socket reachable only by the same UID — it is *not* isolation from the terminal's own children. **(inferred)** Excluding a same-user terminal child would instead require a separate UID/namespace or a `remote_control_password` authorization policy.
