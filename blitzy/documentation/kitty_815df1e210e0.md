...
There is **no** NFC/NFD/`unicodedata` normalization anywhere in the screen or parser path. A repository-wide search for `normalize` / `unicodedata` / `NFC` / `NFD` surfaces only an unrelated file-copy module, never the drawing path. The parser hands the **raw decoded codepoint run** straight to the screen (see §3 below). Consequently the bytes a program sends are stored as-is (subject only to combining classification and width), with no canonical re-composition or decomposition that could merge or reorder the family emoji's codepoints.
