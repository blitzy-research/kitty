...
The `setup.py` `build` step [setup.py:1084] calls `build_static_kittens` [setup.py:1130], which calls `update_go_generated_files` [setup.py:1102] → `subprocess.run([kitty_exe, '+launch', os.path.join(src_base, 'gen/go_code.py')], …)` [setup.py:1112] to generate Go sources and the embedded `data_generated.bin` blobs **before** the Go compiler runs [setup.py:1144].
...
The diff kitten's own `Config`/`Options` Go types live under `kittens/diff/options/`, which is **absent from the working tree** until generation — `.gitattributes` marks `kittens/diff/options/types.py` [.gitattributes:15] and `kittens/diff/options/parse.py` [.gitattributes:16] as `linguist-generated`.
...
```text
$ ./kitty/launcher/kitten diff /tmp/.../left /tmp/.../right   # (captured via PTY, ANSI-stripped)
...
- The ignore patterns come from `conf.Ignore_name`, passed to both `walk` calls [collect.go:299, collect.go:303]. Its config option is `+ignore_name` with an empty base default [kittens/diff/main.py:56]; the shipped `diff.conf` defaults add `.git` [main.py:63], `*~` [main.py:64], and `*.pyc` [main.py:65]. Matching is by basename via `filepath.Match` in `allowed` [collect.go:230].
- The number of context lines around a change defaults to `num_context_lines` = `'3'` [kittens/diff/main.py:37].
- Directories themselves are skipped as entries (only files are keyed); an ignored directory is pruned with `fs.SkipDir` inside `walk` [collect.go:260].

---

## Q2 — Rename recognition: rename vs. delete + add

**Observed.** The identical‑content file that lives at `original_name.txt` on the left and `renamed_name.txt` on the right is rendered as a **single paired header** (left name → right name), i.e. a rename — while the add‑only and remove‑only files each get a **solo** header:

```text
# rename: both names on ONE header row
original_name.txt renamed_name.txt

# contrast — add and remove appear alone:
added.txt      ... This file was added
removed.txt    ... This file was removed
```
...
**Observed.** Repeated diffs over the same tree are served from in‑memory caches; the seven caches are all created together at startup. The pipeline visibly reads each file's bytes once and derives lines and highlighted lines from that single read — the same `changed.txt` bytes back the diff hunk, the line numbering, and the highlighted rendering in one run:

```text
$ ./kitty/launcher/kitten diff /tmp/.../left /tmp/.../right   # (PTY, ANSI-stripped)
@@ -1,4 +1,5 @@
1  line one
...
**Observed (design works at small/mixed scale).** For the mixed fixture, syntax‑highlighted output renders correctly and is byte‑for‑byte deterministic across runs (the `CHANGED` token is highlighted distinctly, no interleaving/garbling), which is the visible signature of correct value‑level isolation.
...
**REPORT‑VERBATIM #1's real consequence (observed — reported exactly, not "corrected").** Value‑level isolation is **not** the same as memory‑safety on the shared map. Because `highlighted_lines_cache.Set` guards the map with only a **read** lock (`RLock`, [tools/utils/cache.go:33]) and then writes `self.data[key] = val` [cache.go:34], concurrent `Set` calls from many highlight goroutines write the same Go map at once. At scale this trips Go's built‑in concurrent‑map‑write detector. Running the **real** `kitten diff` on 250 changed files in built‑in mode produced, on one run, a genuine crash:
...
kitty/tools/utils.(*LRUCache[...]).Set(0x133, {0xc0002b5940?, 0x0?}, {0xc0037ae008?, 0x8, 0x200})
	tools/utils/cache.go:34 +0x85
kitty/kittens/diff.highlight_all.func1(0xc0007da000)
	kittens/diff/highlight.go:224 +0xae
kitty/tools/utils/images.(*Context).Parallel.func1()
	tools/utils/images/utils.go:52 +0x4e
created by kitty/tools/utils/images.(*Context).Parallel in goroutine 152
	tools/utils/images/utils.go:50 +0xe5
```

This crash is **intermittent** — it did not recur across subsequent batches (0/5 at 250 files, 0/10 at 1000 files, 0/20 with varied timing) — which is itself the hallmark of a data race on the map that `Set` guards with only `RLock`.

**Deterministic corroboration (labeled — Go race detector on the REAL cache + REAL pool, via a scratch module; not the `kitten diff` entry point).** To confirm the mechanism deterministically, a scratch Go module outside the repo (`replace kitty => <repo>`) linked the **actual** `kitty/tools/utils` `LRUCache.Set` and the **actual** `kitty/tools/utils/images` `Context.Parallel`, and drove them exactly as `highlight_all` does (distinct index → distinct path → single `Set`). Under `go run -race` it flagged the race on 3/3 runs:
...
Write at 0x... by goroutine 9:
  ...
  kitty/tools/utils.(*LRUCache[go.shape.string,go.shape.[]string]).Set()
      tools/utils/cache.go:34 +0xa4
  ...
  kitty/tools/utils/images.(*Context).Parallel.func1()
      tools/utils/images/utils.go:52 +0x8d
Previous write at 0x... by goroutine 13:
  ... tools/utils/cache.go:34 ...
```
...
**Chroma sibling detail.** Highlighting uses `github.com/alecthomas/chroma/v2 v2.14.0` [go.mod:7]; it selects a lexer by filename/aliases then content analysis, and the per‑token formatter `ansi_formatter` [kittens/diff/highlight.go:84] maps each token to a terminal SGR escape using `SGR_PREFIX = "\033["` [highlight.go:85] and `SGR_SUFFIX = "m"` [highlight.go:86]. The default style is memoized once with `sync.OnceValue` in `DefaultStyle` [highlight.go:26]. The backing regex engine is `github.com/dlclark/regexp2 v1.11.0` [go.mod:9].
...
**Observed.** In one directory run, the plain‑text file is line‑diffed, the binary and the non‑UTF‑8 file are shown as `Binary file: <size>`, and the image is shown with its dimensions and an async "Loading image…" placeholder that is then replaced by an actual in‑terminal image (Kitty graphics‑protocol data):

```text
$ ./kitty/launcher/kitten diff /tmp/.../left /tmp/.../right   # (PTY, ANSI-stripped)
changed.txt   @@ -1,4 +1,5 @@              # text -> line diff
blob.bin      Binary file: 256 B          # random bytes -> binary
invalid.dat   Binary file: 6 B            # non-UTF-8 -> binary
pic.png       Dimensions: 4x4 Size: 73 B  # image -> metadata
pic.png       Loading image...            # image -> async load
```

The non‑UTF‑8 fixture really is invalid UTF‑8 (so it is classified as binary rather than text):

```text
$ python3 -c "open('/tmp/.../left/invalid.dat','rb').read().decode('utf-8')"
utf8_valid=False ('utf-8' codec can't decode byte 0xff in ...)
```

And the image path really emits Kitty graphics‑protocol packets (15 graphics APCs captured in one run) in addition to the two `Loading image...` placeholders — i.e. the placeholder is drawn first and the loaded image is drawn on a later re‑render.
...
And the async, progressive rendering is visible: the collection banner appears first, then the diff, then the image loads on a later frame:

```text
Calculating diff, please wait...      # collection phase
@@ -1,4 +1,5 @@                       # DIFF result rendered
Loading image...                      # image not yet ready
<Kitty graphics-protocol packets>     # IMAGE result -> re-render
```
...
**Observed.** The built‑in engine emits a unified‑diff chunk header; all three `diff_cmd` engines agree on the hunk for our change fixture:

```text
$ ./kitty/launcher/kitten diff -o diff_cmd=builtin left/changed.txt right/changed.txt   -> @@ -1,4 +1,5 @@
$ ./kitty/launcher/kitten diff -o diff_cmd=diff    left/changed.txt right/changed.txt   -> @@ -1,4 +1,5 @@
$ ./kitty/launcher/kitten diff -o diff_cmd=git     left/changed.txt right/changed.txt   -> @@ -1,4 +1,5 @@
```

**Cause→effect (the anchored / "patience" diff).** The default engine is a built‑in **anchored diff** copied from the Go standard library — the header reads `// Copied from the Go stdlib, with modifications.` [kittens/diff/diff.go:1] with the upstream URL [diff.go:2]. Its doc comment [diff.go:21‑48] explains the algorithm: rather than minimizing all inserted/removed lines (worst‑case quadratic), it minimizes the number of **"unique" lines** — lines that appear exactly once in *both* old and new — and uses those unique lines to **anchor** the matching regions [diff.go:32‑35]. It "guarantees to run in O(n log n) time" [diff.go:39] "instead of the standard O(n²) time" [diff.go:40], and the comment notes some systems call this a "patience diff" [diff.go:42] (a name it explicitly avoids). This matches the upstream Go `internal/diff` package the file is copied from (validated against upstream documentation).
...
(Empty again — clean.) All temporary observation artifacts lived **outside** the repository tree under `/tmp` — the fixture pair (`/tmp/kitty_diff_fix.*`), the pristine pre‑generation tree (`/tmp/kitty_pregen.*`), the `NumCPU` scratch program (`/tmp/numcpu_probe`), the `-race` corroboration module (`/tmp/kitty_racetest.*`), and the PTY/strip helper scripts — and were all removed with `rm -rf` after the observations were captured. The generated build products (`data_generated.bin`, root `constants_generated.go`, the generated `kittens/diff/options/` tree) are git‑ignored/untracked, so they never appear in `git status --porcelain`.
