...
**Branch / commit:** `kitty_815df1e210e0` — `git rev-parse HEAD` → `815df1e210e0a9ab4622f5c7f2d6891d7dbeddf1`.
...
Every behavioral claim below is placed next to the **actual, unedited runtime output** that demonstrates it. Output was produced through kitty's **real** input path (raw CSI bytes → VT parser → live `Screen` → key encoder), never through a remote-control hook, debug hook, mock, or synthetic bypass. Where a fact comes from reading the source rather than from captured output, it is grounded in a `file:line` citation; the single value that is a code fact rather than a documented number (the capacity **8**) is called out explicitly.
...
**Direct answer.** The push limit is exactly **8** entries per buffer. Pushing more than 8 does **not** raise an error and is **not** rejected — the **oldest** entry is silently **evicted** (FIFO). Exhausting one buffer's stack does **not** affect the other buffer's stack.
...
- *Cross-buffer isolation under exhaustion* follows directly from the two **separate** arrays (`kitty/screen.h:128`) and the pointer-only toggle (`kitty/screen.c:1079,1086`): operations on main touch only `main_key_encoding_flags`, so `alt_key_encoding_flags` cannot change no matter how hard main is exercised.

> **Code-derived cross-check (inferred).** Porting the exact `screen.c` push / current / pop algorithm to plain Python reproduces the same capture — `push 1..8 → [8,7,6,5,4,3,2,1,0,0,0]` and `push 1..9 → [9,8,7,6,5,4,3,2,0,0,0,0]` — corroborating that the observed FIFO/capacity behavior is exactly the array-slide in `screen_push_key_encoding_flags`. This bullet is a code-derived corroboration; the primary evidence is the runtime capture above.
...
**Cross-check (observed):** encoding `Ctrl+Shift+a` directly and comparing against the hand-computed CSI string confirmed the arithmetic:
...
**Direct answer.** The per-screen stack **isolation itself does not break down** under any observed condition. What *does* depend on terminal modes/settings are four distinct behavioral differences, each covered below with its own evidence: (a) **which DEC mode** performs the buffer switch, (b) the **empty/over-pop reset** to legacy `0`, (c) **flag-dependent encoding** of legacy-representable keys, and (d) the **query response**.
...
- `CSI = u` → `screen_set_key_encoding_flags` with `how = 1` (set) (`kitty/vt-parser.c:1229`).
...
**Summary for OBJ-5.** No observed condition caused the two stacks to leak into each other or the isolation to break down. The only mode/setting-dependent *differences* are the four itemized above — and of those, only (c) changes the bytes a key press produces, while (a) changes cursor/screen side effects but leaves stack isolation intact.
