...
### CMD: python3 -c urllib HEAD https://download.calibre-ebook.com/ci/kitty/linux-64.tar.xz
HTTP 200
...
**Causal interpretation [inferred]:** the newer bundle's protocol XML is *newer than the pinned source expects*, and `-Werror=switch` turns the resulting `enumeration value … not handled in switch` warning into a fatal error. The source at commit `815df1e2` is unmodified (read-only rule); the mismatch is between the rolling dependency bundle and the pinned source, i.e. environmental drift rather than a defect at this commit. (The mechanics in (1) and (2) are observed; only this "drift, not defect" framing is the inferred interpretation.)

### R1.3 Why a commit-compatible bare build is not obtainable here

The canonical dependency source is the single rolling `BUNDLE_URL` above; there is no version-pinned or archived bundle for this commit (the URL has no version parameter, and the live server returns only the current, drifted bundle — the `HTTP 200` HEAD in §R1.2). Consequently a *successful bare default build* cannot be reproduced in this environment without either modifying the pinned source (prohibited by the read-only rule) or relaxing the default `-Werror` policy (which makes it no longer the bare default build). **R1's strict "default build succeeds" requirement is therefore marked NOT satisfiable at this commit in this environment [observed], and no default-build-success claim is made.**
...
===== kitty/fonts/list.py:30-45 (secondary invocation path) =====
...
**[observed via source read at commit `815df1e2`]** — this path re‑enters the identical `kitten choose-fonts` entry point (confirmed: the container `git rev-parse HEAD` printed above is `815df1e210e0a9ab4622f5c7f2d6891d7dbeddf1`), so all behavior documented above applies unchanged.
...
| **R1** build‑drift cause | rolling `BUNDLE_URL` ships newer `wayland-protocols`; `-Werror=switch` fatalizes an unhandled enum in `glfw/wl_window.c:668` | inferred (mechanics observed; "drift not defect" framing inferred) | `ci.py:16`/`bypy/devenv.go:257` (URL); `glfw/wl_window.c:668` | §R1.2 (1)(2)(causal) | Env drift vs. pinned source, not a defect at this commit |
...
| **R3a** `choose_fonts` alias | **visible** alias (not hidden) | visibility observed; mechanism inferred | `clone.Hidden=false` `kittens/choose_fonts/main.go:97`; `clone.Name` `:98` | §R3.1 | Both spellings on root listing + byte‑identical `--help` |
...
| **R4 fresh‑process lifecycle** | terminate #1 → `kill -0` "No such process" → same config dir → **fresh** `kitty` (new PID) → invoke → read‑back; control (delete file) → default returns | observed | startup loader `kitty/cli.py:1089-1093`,`:1067-1068`; `config.py:163`; `conf/utils.py:322-329`; `constants.py:87-89,131-133` | §R4.3 | Decisive: a *new* process reads the saved value; control proves it came from the file |
